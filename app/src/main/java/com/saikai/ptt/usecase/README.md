...
**只放已经能做的事。** Task39 的 `HistoryUseCases` 里没有搜索、没有删除、没有清理——
那些是 Task40 的，写一个转手就返回空的 `SearchHistory` 比不写更糟（`CLAUDE.md` §38）。
Task40 把它们补齐了，但规则不变。

**搜索不是单独一个 UseCase。** `ObserveHistory(query)` 里空串就是全部历史
（`05_DataModel §28`）。分成两个意味着界面有一个「是否在搜索」的模式要和屏幕上的东西
保持同步，而那是个可以不存在的状态。

由 **Task32** 建立，**Task33** 加入用户名相关动作，**Task34** 权限，**Task35** 设置，
**Task39** 历史读取与已读/收藏，**Task40** 搜索、删除、清理、保留期与占用统计，
后续任务按需扩充（ASR）。
