...
**停止有两种，区别是听得见的。** `stop()` 立即丢弃设备缓冲里的音频，
`drainAndStop()` 放完再停。用哪一个由会话如何结束决定（Task29）：收到 VOICE_END 是
对方说完了，那段尾音是一句话的最后一个词；被强插、掉网、来电抢焦点则是传输被切断，
再把它放完只会推迟顶替它的那一位——而在强插这条路径上，那段等待发生在仲裁会话所有权
的锁里面。

**`drainAndStop()` 会等已经排队的音频放完。** `AudioTrack.stop()`
...
`RecordingPlayback` 与上面所有东西**没有关系**。它放的是硬盘上一个已经写完的
Ogg/Opus 文件，用的是 `MediaPlayer`，走 `USAGE_MEDIA`（ADR-008）——不是
`AudioTrack`，不是 `USAGE_VOICE_COMMUNICATION`，也不共用 `AndroidAudioFocus`。
通话音频是一条实时管线，回放是放一个文件；把它们塞进同一个类只会让两边都要处理对方
的状态。

**语音优先，但不是靠 AudioFocus 实现的。** Task39 要求回放期间收到 PTT 时语音赢。
焦点告诉这个播放器「有东西抢走了音频」，而通知提示音也会触发它——为了一声提示音丢掉
用户在录音里的位置是错的。所以真正停掉回放的是
`ui.history.HistoryViewModel` 监听的会话状态：**只为那件必须赢的事让路**。
焦点照样申请，为的是音量键落在用户以为的那条流上。

**全部在主线程。** `MediaPlayer` 不是线程安全的，而这里只有两个东西碰它：按钮和
进度轮询。容器给它的是主线程 scope（`di/AppContainer.kt`），于是不需要任何锁，
也不存在「`stop()` 刚 release 完，轮询正好问它在不在放」这一类竞态。
一秒读五次播放位置对主线程不算开销。唯一的例外是 `exists()`——它只碰文件系统，
由列表的 flow 在 `Dispatchers.IO` 上调用。
