...
实现持久化 UUID v4 Device ID。

## Requirements

- Generate exactly once on first initialization.
- Store through DataStore.
- Stable across app restarts and device reboots.
- Independent from IP address.
- Do not use MAC/IMEI/serial.

## Acceptance Criteria

- Valid UUID v4 is generated.
- Repeated reads return the same ID.
- Unit tests cover first creation and persistence.
