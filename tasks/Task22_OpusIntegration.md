...
- low-latency suitable configuration
- bitrate/sample/frame parameters documented
- lifecycle safe
- minimal allocations

Before selecting a native library, evaluate maintenance and compatibility.

If a native dependency is needed, document the decision in an ADR.

## Acceptance Criteria

PCM frame → Opus → PCM round-trip works in automated tests.
