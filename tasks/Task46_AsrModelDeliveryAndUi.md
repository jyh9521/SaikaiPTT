...
- 四个文件分别下载，**不是一个归档**（`ADR-012 §2`：Java 无内置 bzip2，
  而 zip 会让设备峰值多占一份）
- 下到临时目录 → **逐个校验 SHA-256 与预期大小** → 整个目录改名到位。
  原子性靠这个顺序，不靠归档
- 任一文件校验不符：删除整个临时目录，要求重新下载
- 下载失败 / 用户取消：ASR 保持关闭，**不影响任何其它功能**
- 可中断、可重来；进度按总字节数算，不是「第几个文件」
- 状态写入 `asr_model_ready`
- 预期摘要与字节数钉在 `SaikaiConfig`，取自 `ADR-012 §同一性与安全审查` 的表
