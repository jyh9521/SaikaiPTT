...
Status: **部分被 ADR-012 取代**（2026-09-16）
Date: 2026-09-16
Supersedes: ADR-006（引擎选型与「约 50 MB 一次下载」两条）

> **本文件的 Decision（v1 不集成）已被 `ADR-012` 推翻。**
> 推翻的理由是本文件的体积论证口径错了：713 MB 是 `.tar.bz2` 压缩包，
> 其中 82% 是移动端不加载的 fp32 权重与测试音频，真正需要的 int8 子集是
> **153 MiB**。
>
> **仍然有效的部分**：对 Vosk 的判定（NDK 25、无 `max-page-size`、16 KB 不过关，
> 且无法从项目侧绕过）、对 sherpa-onnx 16 KB 对齐的二进制级验证、
> 以及各引擎的许可证结论。`ADR-012` 直接引用这些，不重复推导。
